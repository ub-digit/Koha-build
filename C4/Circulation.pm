...
sub CanBookBeIssued {
    ...
    } elsif ( my $num_overdues = $patron->has_overdues(1) ) {
        ...
        if ( $actualissue and $actualissue->borrowernumber eq $patron->borrowernumber
            ...
            if ( $itemtype_object ) {
                ...
                if ( $accumulate_charge > 0 ) {
                    ...
                    $item_unblessed->{charge} = $charge; # TODO: WTF?
                }
            }
...
sub CanBookBeRenewed {
    ...
    unless ( $override_limit ){
        ...
        my $hasoverdues = $patron->has_overdues(1);
...
sub _CanBookBeAutoRenewed {
    ...
    if ( $patron->is_expired(1) && $patron->category->effective_BlockExpiredPatronOpacActions ) {
    ...
    my $soonest = GetSoonestRenewDate($patron, $item, $issue);
    if ( $soonest > dt_from_string() )
    {
