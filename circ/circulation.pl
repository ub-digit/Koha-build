  ...
  for my $barcode ( @$barcodes ) {
    ...
    if( $item and ( !$blocker or $force_allow_issue ) ){
        ...
        unless($confirm_required) {
            ...
            my $issue = AddIssue( $patron->unblessed, $barcode, $datedue, $cancelreserve, undef, undef, { onsite_checkout => $onsite_checkout, auto_renew => $session->param('auto_renew'), switch_onsite_checkout => $switch_onsite_checkout, cancel_recall => $cancel_recall, recall_id => $recall_id, note => $note } );
