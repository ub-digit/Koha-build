...
subtest "CanBookBeRenewed tests" => sub {
    ...
    $fines = Koha::Account::Lines->search( { borrowernumber => $renewing_borrower_obj->borrowernumber, itemnumber => $item_7->itemnumber } );
    $fines->delete();
    ...
    subtest "auto_account_expired | BlockExpiredPatronOpacActions" => sub {
        ...
        ( $renewokay, $error ) =
          CanBookBeRenewed( $renewing_borrower_obj->unblessed, $issue );
        is( $renewokay, 0, 'Do not renew, renewal is automatic' );
    ...
    ( $renewokay, $error ) = CanBookBeRenewed( $renewing_borrower_obj, $recall_issue );
    is( $error, 'recalled', 'Cannot renew item if biblio is recalled and has no item allocated' ); # TODO: Stale issue problems? Check these tests
    $recall->set_cancelled;
